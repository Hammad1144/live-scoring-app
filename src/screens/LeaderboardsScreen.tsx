...
type Board = 'runs' | 'sixes' | 'wickets' | 'economy';
export function LeaderboardsScreen({ onBack }: { onBack: () => void }) {
  const db = useSQLiteContext();
  const [board, setBoard] = useState<Board>('runs');
  const [data, setData] = useState<{ topScorers: LeaderboardRow[]; mostSixes: LeaderboardRow[]; mostWickets: LeaderboardRow[]; bestEconomy: LeaderboardRow[] } | null>(null);
...
const styles = StyleSheet.create({ container: { padding: 18, backgroundColor: colors.bg, minHeight: '100%', paddingBottom: 40 }, chips: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 }, row: { marginBottom: 9, flexDirection: 'row', alignItems: 'center', gap: 12 }, rank: { width: 36, height: 36, borderRadius: 18, backgroundColor: colors.surface2, alignItems: 'center', justifyContent: 'center' }, rankText: { color: colors.primary, fontWeight: '900' }, name: { color: colors.text, fontWeight: '800', fontSize: 16 }, secondary: { color: colors.muted, marginTop: 3, fontSize: 11 }, value: { color: colors.primary, fontWeight: '900', fontSize: 22 } });
